...
const App: React.FC = () => {
  ...
  const addProject = async (title: string, clientId: string, propertyId: string, professionalId: string, serviceId: string, registryId: string, deadline?: string, certificationNumber?: string) => {
    const uid = userIdRef.current;
    if (!uid) return;
    try {
      const { data, error } = await supabase.from('projects').insert({ title, client_id: clientId, property_id: propertyId, professional_id: professionalId, service_id: serviceId, registry_id: registryId, deadline: deadline || null, certification_number: certificationNumber || null, current_step_index: 0, user_id: uid }).select().single();
      if (error) throw error;
      if (data) {
    ...
    } catch (err: any) { showNotification(`Erro: ${extractErrorMessage(err)}`, 'error'); }
  };
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <main className="flex-1 overflow-hidden flex flex-col pt-4">
          <div className="flex-1 bg-white rounded-tl-[var(--content-radius)] shadow-sm overflow-y-auto custom-scrollbar relative">
            ...
            <div className="px-6 sm:px-10 py-8 relative z-10 min-h-full">
              {dbError && (
