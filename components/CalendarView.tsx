...
import { Appointment, Client, Project, UserTask } from '../types';
...
interface CalendarViewProps {
    ...
    onSaveTask: (task: any, id?: string) => void;
...
const CalendarView: React.FC<CalendarViewProps> = ({
    ...
    const handleOpenTaskModal = (task?: UserTask) => {
        if (task) {
            setEditingTask(task);
            setTaskFormData({
                title: task.title,
                description: task.description || '',
                deadline: task.deadline ? formatDateForInput(parseISO(task.deadline)) : '',
                status: task.status
            });
        } else {
            setEditingTask(null);
    ...
    const toggleTaskStatus = (task: UserTask) => {
        onSaveTask({
            ...task,
    ...
    const renderTaskSection = (title: string, taskList: UserTask[]) => (
        <div className="flex flex-col gap-6 bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-200/60 p-8 flex-1 overflow-hidden h-full">
            <div className="flex items-center justify-between shrink-0">
                <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1.5 flex items-center gap-2">
                        {title.includes('Pendente') ? <AlertTriangle size={12} className="text-amber-500" /> : <History size={12} className="text-primary" />}
                        {taskList.length} tarefas encontradas
                    </p>
                </div>
            </div>
            ...
            {taskList.length > 0 ? (
                <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
                    {taskList.map(task => (
                        <div key={task.id} className="group p-6 bg-slate-50/50 rounded-2xl border border-transparent hover:border-primary/40 hover:bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all">
                            <div className="flex justify-between items-start gap-4">
                                <div className="flex-1">
                                    <h4 className={`text-sm font-bold text-slate-800 mb-2 leading-tight ${task.status === 'completed' ? 'line-through text-slate-400' : ''}`}>
                                        {task.title}
                                    </h4>
                                    {task.description && (
                                    ...
                                    {task.deadline && (
                                        <div className="flex items-center gap-3">
                                            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${new Date(task.deadline) < new Date() && task.status === 'pending' ? 'bg-rose-50 text-rose-600 border border-rose-100 animate-pulse' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}>
                                                <Clock size={12} />
                                                {formatLocal(parseISO(task.deadline), { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                                            </div>
                                            {new Date(task.deadline) < new Date() && task.status === 'pending' && (
                                                <span className="text-[8px] font-black text-rose-600 uppercase tracking-widest">Atrasada!</span>
                                            )}
                                        </div>
                                    )}
                                </div>
                                <div className="flex gap-2 shrink-0">
                                    <button
                                        ...
                                        className={`p-3 rounded-xl transition-all shadow-sm ${task.status === 'completed' ? 'bg-primary text-white shadow-primary/20' : 'bg-white text-slate-400 hover:text-primary hover:border-primary/20 hover:scale-110 border border-slate-200'}`}
                                    ...
                                    <button
                                        ...
                                        className="p-3 bg-white text-slate-400 hover:text-primary rounded-xl border border-slate-200 shadow-sm transition-all hover:scale-110"
                                    >
                                        <Plus size={16} className="rotate-45" />
                                    </button>
                                    <button
                                        onClick={(e) => { e.stopPropagation(); onDeleteTask(task.id); }}
                                        className="p-3 bg-white text-slate-400 hover:text-rose-500 rounded-xl border border-slate-200 shadow-sm transition-all hover:scale-110"
                                    >
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center flex-1">
                    ...
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Nenhuma tarefa {title.toLowerCase()}</p>
                </div>
            )}
        </div>
    );
        ...
        <div className="flex flex-col gap-6 animate-in fade-in duration-500 overflow-hidden">
                ...
                <div className="flex items-center gap-6">
                    <h2 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight flex items-center gap-3">
                        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary rounded-xl flex items-center justify-center shadow-lg shadow-primary/10 text-white shrink-0">
                            {viewMode === 'calendar' ? <CalendarDays size={20} /> : <ListChecks size={20} />}
                        </div>
                        <div className="flex flex-col overflow-hidden">
                            <span className="leading-none truncate">{viewMode === 'calendar' ? 'Agenda de Serviços' : 'Gerenciador de Tarefas'}</span>
                    ...
                    <div className="hidden sm:flex bg-slate-100 p-1 rounded-xl border border-slate-200 ml-4">
                        <button
                            onClick={() => setViewMode('calendar')}
                            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === 'calendar' ? 'bg-white text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            Calendário
                        </button>
                        <button
                            onClick={() => setViewMode('tasks')}
                            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${viewMode === 'tasks' ? 'bg-white text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        >
                            Tarefas
                        </button>
                    </div>
                ...
                <div className="flex gap-3">
                    {isMobile && (
                        <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 flex-1">
                            <button
                                onClick={() => setViewMode('calendar')}
                                className={`flex-1 py-3 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${viewMode === 'calendar' ? 'bg-white text-primary shadow-sm' : 'text-slate-500'}`}
                            >
                                Calendário
                            </button>
                            <button
                                onClick={() => setViewMode('tasks')}
                                className={`flex-1 py-3 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${viewMode === 'tasks' ? 'bg-white text-primary shadow-sm' : 'text-slate-500'}`}
                            >
                                Tarefas
                            </button>
                        </div>
                    ...
                    <button
                        onClick={() => viewMode === 'calendar' ? handleOpenModal() : handleOpenTaskModal()}
                    ...
                    >
                        ...
                        <span className="relative z-10">{viewMode === 'calendar' ? 'Novo Agendamento' : 'Nova Tarefa'}</span>
                    </button>
                </div>
            </div>
            ...
            {viewMode === 'calendar' ? (
                ...
                <div className="flex-1 overflow-hidden">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full overflow-y-auto lg:overflow-hidden p-1">
                        {renderTaskSection("Tarefas Pendentes", tasks.filter(t => t.status === 'pending'))}
                        {renderTaskSection("Histórico de Concluídas", tasks.filter(t => t.status === 'completed'))}
                    </div>
                </div>
            )}
                    ...
                    <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-300">
                        <div className="p-8 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-2xl bg-primary/10 text-primary font-black flex items-center justify-center shadow-inner">
                                    <ListChecks size={24} />
                                </div>
                                <div>
                                    <h3 className="text-lg font-black text-slate-800 tracking-tight">{editingTask ? 'Editar Tarefa' : 'Nova Tarefa'}</h3>
                                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-1">Organização de Fluxo</p>
                                </div>
                            </div>
                            <button onClick={() => setShowTaskModal(false)} className="p-3 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-2xl transition-all">
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleTaskSubmit} className="flex flex-col">
                            <div className="p-8 space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-50 border-2 border-slate-100/50 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all placeholder:text-slate-300"
                                        placeholder="Ex: Revisar relatório final..."
                                    />
                                </div>

                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Prazo de Conclusão</label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full bg-slate-50 border-2 border-slate-100/50 rounded-2xl pl-16 pr-6 py-4 text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full bg-slate-50 border-2 border-slate-100/50 rounded-2xl px-6 py-4 text-sm font-bold text-slate-700 focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all min-h-[120px] placeholder:text-slate-300"
                            ...
                            <div className="p-8 bg-slate-50 border-t border-slate-200 flex gap-4">
                                <button
                                    ...
                                    className="flex-1 px-8 py-4 bg-white text-slate-600 rounded-2xl font-bold text-xs uppercase tracking-[0.2em] hover:bg-slate-100 transition-all border-2 border-slate-200"
                                >
                                    Cancelar
                                </button>
                                <button
                                    type="submit"
                                    className="flex-1 px-8 py-4 bg-primary text-white font-black uppercase tracking-[0.2em] text-xs rounded-2xl hover:bg-primary-dark shadow-2xl shadow-primary/20 transition-all active:scale-95"
                                >
