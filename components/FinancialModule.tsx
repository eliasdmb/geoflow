...
const FinancialModule: React.FC<FinancialModuleProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    project_id: '',
    account: '',
    from_account_id: '', // New field for transfers
    to_account_id: '', // New field for transfers
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSaveTransaction({
      ...
      from_account_id: formData.from_account_id || undefined, // Include for transfers
      to_account_id: formData.to_account_id || undefined,     // Include for transfers
    ...
    setFormData({
      ...
      project_id: '',
      account: '',
      from_account_id: '', // Clear for next transaction
      to_account_id: '',     // Clear for next transaction
      scope: 'Empresa',
      notes: ''
    });
  };
        ...
        <div className="flex flex-col xl:flex-row xl:items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-primary text-white rounded-xl hover:bg-primary-dark shadow-md shadow-primary/10 text-xs font-black uppercase tracking-widest transition-all"
            >
              <Plus size={16} /> {activeTab === 'FLUXO' ? 'Novo Lançamento' : 'Nova Despesa Card'}
            </button>
            <button
              onClick={() => setShowAccountModal(true)}
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-md shadow-blue-200/50 text-xs font-bold transition-all"
            >
        ...
        showCardModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl p-8">
              <h3 className="text-xl font-black text-slate-800 mb-6">Novo Cartão</h3>
              <form onSubmit={e => { e.preventDefault(); onSaveCreditCard({ name: cardFormData.name, credit_limit: parseFloat(cardFormData.credit_limit), closing_day: parseInt(cardFormData.closing_day), due_day: parseInt(cardFormData.due_day) }); setShowCardModal(false); }} className="space-y-4">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setShowAccountModal(false)} className="text-slate-500 hover:text-slate-700">
                  <X size={20} />
                </button>
              </div>
